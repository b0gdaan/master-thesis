...
DCC-GARCH never passes: its average $\bar{\beta} = 1.44$ means that realized
...
movements rather than overstating them. This is precisely the over-smoothing
of DCC-GARCH seen throughout the main results.
...
First, the seven \emph{fitted} models are tested against two benchmarks: the DCC-GARCH
...
average-RMSE ranking of Table~\ref{tab:model_ranking_summary}, in which AR(1), HAR, and
Ridge are statistically indistinguishable. Second, the Diebold--Mariano test assumes
...
Naive\_Last, and by extension any test against AR(1), are accordingly best read as
...
AR(1) against DCC-GARCH --- rather than proving the methods correct in an absolute sense.
