...
AR(1) and HAR pass the efficiency test at the same rate (70.8\% of experiments),
...
DCC-GARCH never passes: its average $\bar{\beta} = 1.44$ means that realised
...
performance gradient is steep. Ridge ($F = 4.76$), AR(1) ($F = 5.07$), and HAR
...
The autocorrelation results follow a clear gradient. Naive (last value) and the
autoregressive models (AR(1), HAR) partly capture the serial structure,
...
the tree ensembles (e.g.\ $Q > 1{,}000$ for XGBoost on most pairs) drive the
...
is of the order of $10^{-4}$, so the practical effect on $p$-values is
negligible.

Applied to all 336 DM comparisons (168 ML models vs DCC-GARCH and 168 vs
Naive (last value), Table~\ref{tab:dm_hln}) the HLN correction produces zero sign
changes in significance at the 5\% level (average correction factor
$k = 0.999774$, consistent with the slight reduction of every HLN statistic
relative to its uncorrected counterpart in Table~\ref{tab:dm_hln}).
...
\begin{itemize}
    ...
    \item \textbf{Model Metrics} --- RMSE $>0$, $R^2 \leq 1$, MAE $\leq$ RMSE,
          all windows and expected models present, AR(1) beats DCC-GARCH on average.
