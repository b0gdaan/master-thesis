...
The implementation is Python only, with the exact versions pinned in \texttt{requirements.txt}. The regularised linear models, tree ensembles, logistic classifier, and the walk-forward splitting utilities come from scikit-learn \cite{sklearn2026}; gradient boosting on the GPU uses XGBoost \cite{xgboostdocs2026}; the univariate GARCH(1,1) fits underlying the DCC benchmark use the \texttt{arch} package \cite{archdocs2026}. Market data is retrieved through \texttt{yfinance} \cite{yfinance2026}. The document itself is typeset with the \texttt{finthesis} class of the \texttt{finlatex} project \cite{finlatex2026}, which supplies the university title pages and layout. Beyond the pinned versions, reproducibility rests on the fixed random seed (\texttt{random\_state = 42}, mirrored for XGBoost) and on the frozen return file described above; no step draws on system time or on an unpinned remote source.
...
\begin{enumerate}
    \item \textbf{\texttt{01\_EDA\_Dataset.ipynb}} --- Exploratory data analysis. Produces normalised price trajectories, rolling volatility profiles, a pairwise correlation heatmap, rolling pairwise correlation time series, and the Fisher-transformation comparison plot. These figures appear in Appendix~A.

    \item \textbf{\texttt{02\_GridSearch.ipynb}} --- Hyperparameter search diagnostics. Reports time-series cross-validation (expanding-window \texttt{TimeSeriesSplit}) RMSE surfaces for the representative Bitcoin--S\&P~500 experiment, regularisation-path plots for Ridge and ElasticNet, and Random Forest feature-importance charts. Results support the model-selection discussion in Chapter~3.
    ...
    \item \textbf{\texttt{04\_DM\_Tests\_Visuals.ipynb}} --- Diebold--Mariano test visualisation. Produces the DM heatmap comparing the best machine-learning model against DCC-GARCH (Figure~\ref{fig:dm_heatmap}) and the representative out-of-sample forecast panel for the Bitcoin--S\&P~500 pair (Figure~\ref{fig:sp500_forecast}).
...
\begin{itemize}
    ...
    \item Feature scaling was added for the regularised linear models.
    ...
    \item Notebook imports and notebook-specific plotting logic were stabilised for presentation and interpretation.
\end{itemize}
