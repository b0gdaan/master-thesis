...
\begin{table}[H]
    ...
    \begin{tabular}{lrrr}
        ...
        \bottomrule
    \end{tabular}
    \par\smallskip\noindent\footnotesize{\textit{Note: Results averaged across all asset-pair and window-length combinations. RMSE and R$^2$ computed on out-of-sample walk-forward predictions. HAR = Heterogeneous AutoRegressive model; Ensemble = adaptive inverse-RMSE-weighted ensemble. Best-performing model by average RMSE is Ridge (regularised linear regression with StandardScaler). All metrics reflect the full sample through 26 May 2026 (3{,}053 observations).}}
\end{table}

Five notable patterns emerge from Table~\ref{tab:model_ranking_summary}. First, the three top-ranked models --- Ridge, AR1, and HAR --- form a near-indistinguishable cluster with average RMSE values of 0.0656, 0.0659, and 0.0659 respectively and out-of-sample $R^2$ values all at or above 0.9424. The differences within this cluster are economically negligible. Second, the naive persistence model (Naive\_Last) sits just outside the top cluster with RMSE 0.0666, confirming that the dominant forecasting mechanism is autoregressive in nature. Third, the regularised linear models and the adaptive Ensemble occupy a coherent second tier, all achieving $R^2 > 0.93$. Fourth, tree-based methods --- GBM, Random Forest, and XGBoost --- constitute a clearly distinct third tier, with RMSE values between 0.09 and 0.12 and $R^2$ values below 0.90. Fifth, the DCC-GARCH benchmark ranks last by a substantial margin, with an average $R^2$ of 0.3715.

The interpretation is precise. The dependency target is strongly persistent, and that persistence constitutes the principal forecasting signal. Ridge, AR1, and HAR each exploit that persistence effectively but through structurally different mechanisms: AR1 uses a single autoregressive coefficient; HAR uses three temporal scales (daily, weekly, monthly); Ridge uses the full regularised feature set anchored by the most recent lag. That all three converge to essentially the same out-of-sample accuracy is itself a finding: the forecasting problem is well-characterised by linear autoregressive dynamics, and additional model complexity does not extract materially incremental information.
