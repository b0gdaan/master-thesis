...
\begin{table}[H]
    ...
    \begin{tabular}{lrrr}
        ...
        \midrule
        AR1 & 0.0665 & 0.0405 & 0.9423 \\
        Naive\_Last & 0.0672 & 0.0395 & 0.9411 \\
        Ridge & 0.0914 & 0.0636 & 0.8917 \\
        ElasticNet & 0.0921 & 0.0643 & 0.8902 \\
        RF & 0.1009 & 0.0709 & 0.8694 \\
        GBM & 0.1035 & 0.0732 & 0.8630 \\
        XGB\_GPU & 0.1043 & 0.0744 & 0.8605 \\
        DCC\_GARCH & 0.2147 & 0.1687 & 0.3814 \\
        \bottomrule
    \end{tabular}
    \par\smallskip\noindent\footnotesize{\textit{Note: Results averaged across all asset-pair and window-length combinations. RMSE and R$^2$ computed on out-of-sample walk-forward predictions. Best-performing model excluding DCC-GARCH is AR1. All metrics reflect the full sample through 18 April 2026 (3{,}025 observations); minor numerical differences from earlier pipeline runs are attributable to the expanded dataset.}}
\end{table}

Four notable patterns emerge from Table~\ref{tab:model_ranking_summary}. First, the top-ranked model is the AR1 baseline, with an average RMSE of 0.0665 and an out-of-sample $R^2$ of 0.9423. Second, the naive persistence model is nearly indistinguishable from AR1 (RMSE difference of 0.0007), confirming that the dominant forecasting mechanism is autoregressive in nature. Third, the regularized linear models Ridge and ElasticNet occupy an intermediate tier with $R^2$ values near 0.89, outperforming all tree-based ensembles on average. Fourth, the DCC-GARCH benchmark ranks last by a substantial margin, with an average $R^2$ of only 0.3814, indicating that the parametric conditional-correlation model is not competitive in the direct one-step-ahead prediction of the Fisher-transformed rolling correlation target.
...
\begin{figure}[H]
    ...
    \caption{Diebold--Mariano test results comparing the best machine-learning model against DCC-GARCH across all asset-pair and window-length combinations. Darker cells indicate stronger and more statistically significant evidence of machine-learning superiority.}
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrr}
        ...
        \midrule
        Logit & 0.506 & 0.209 & 0.509 \\
        GBM\_Cls & 0.504 & 0.141 & 0.512 \\
        RF\_Cls & 0.500 & 0.030 & 0.517 \\
        \bottomrule
    \end{tabular}
...
The highest individual signal performance is recorded for the Bitcoin--S\&P 500 pair. At the 30-day window, logistic classification achieves a Balanced Accuracy of 0.5361 and an F1 score of 0.2492 for the downside class --- a modest but statistically and practically meaningful departure from the uninformative baseline of 0.50.
...
The signal is designed to function as a probability-guided warning indicator identifying elevated-risk periods, thereby supporting a risk-reduction decision rather than a directional trade. For NASDAQ the performance profile is broadly similar. For precious metals and the dollar-index proxy, performance is more attenuated and less consistent, reflecting the weaker and less stable relationship between crypto dynamics and stress in those markets.
